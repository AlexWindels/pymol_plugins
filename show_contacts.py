...
def show_contacts(selection,selection2,result="contacts",cutoff=3.6, bigcutoff = 4.0, SC_DEBUG = DEBUG):
    ...
    ########################################
    #Highlight the residues that are involved in the polar contacts (the ones
    #for which the dashed lines are drawn): show them as sticks and color them
    #yellow. We grab the donor/acceptor atoms of each selection that lie within
    #the polar-contact cutoff of the other selection, then expand to whole
    #residues with byres.
    contact_residues = result + "_contact_residues"
    if all1_sele_count and all2_sele_count:
        cmd.select(contact_residues,
                   '(byres (all_don_acc1_sele within %f of all_don_acc2_sele)) '
                   'or (byres (all_don_acc2_sele within %f of all_don_acc1_sele))'
                   % (bigcutoff, bigcutoff))
        cmd.show('sticks', contact_residues)
        cmd.color('yellow', contact_residues)
        #remove the named selection so it doesn't clutter the object panel /
        #leave selection indicators on screen (the sticks + color persist)
        cmd.deselect()
        cmd.delete(contact_residues)
